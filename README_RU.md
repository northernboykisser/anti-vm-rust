...
### `.filtered(|| ...)` — Что делать, если проверка не пройдена

По умолчанию при провале любого фильтра программа молча завершается через `exit(0)`. Если передать в `.filtered(...)` функцию, то при провале проверки вместо закрытия программы будет вызвана эта функция.
...
fn main() {
    ProtectionBuilder::new()
        ...
        .set_http(true)
        .filtered(|| {
            // Вместо закрытия программы — своя логика
            println!("Похоже, среда нежелательная!");
        })
        .init();

    // Код продолжит работу, даже если фильтр не пройден
    println!("Программа продолжает работу");
}
```

> **Примечание:** если `.filtered(...)` не задан, поведение остаётся прежним — программа завершится при провале любого фильтра.
